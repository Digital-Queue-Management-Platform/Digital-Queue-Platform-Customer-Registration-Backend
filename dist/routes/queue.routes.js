...
router.get('/outlet/:outletId', async (req, res) => {
    try {
        ...
        if (mongoose.connection.readyState !== 1) {
            console.error('[QUEUE] MongoDB not connected - cannot fetch real queue data');
            return res.status(503).json({
                success: false,
                message: 'Database connection unavailable. Please ensure MongoDB Atlas is connected.',
        ...
        const waitingCustomers = await Customer_1.Customer.find({
            outletId,
            status: 'waiting',
        ...
        const servingCustomer = await Customer_1.Customer.findOne({
            outletId,
            status: 'being_served',
            registrationTime: { $gte: todayStart, $lt: todayEnd }
        }).select('tokenNumber');
        // Get completed customers for analytics
        const completedCustomers = await Customer_1.Customer.find({
            outletId,
            status: 'completed',
            registrationTime: { $gte: todayStart, $lt: todayEnd }
        ...
        let queueRecord = await Queue_1.Queue.findOne({ outletId, date: todayStart });
        if (!queueRecord) {
            queueRecord = new Queue_1.Queue({
                outletId,
                date: todayStart,
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to fetch real queue data from database',
